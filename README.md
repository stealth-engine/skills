...
| [`vercel-preview-password-gate`](./skills/vercel-preview-password-gate/SKILL.md) | Password-protect Vercel **preview** deployments for free, on any framework — a self-contained middleware gate (Next.js `proxy.ts`, or Vercel Routing Middleware for SvelteKit/Nuxt/Astro/static) instead of the paid Advanced Deployment Protection add-on. Humans unlock once via a signed 1-year cookie (only a **scrypt hash** lives in env); automation passes with **named, individually revocable bypass tokens** by header or query param. Keys off `VERCEL_TARGET_ENV` so custom environments (`staging`) are gated too, and the prod build **strips the gate entirely** → zero production cost. Covers set/rotate password, token management, gating production deliberately (coming-soon/client demo), and when to use free Vercel Authentication instead. Ships both templates + hash/token/strip scripts. |
