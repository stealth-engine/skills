...
| [`vercel-preview-password-gate`](./skills/vercel-preview-password-gate/SKILL.md) | **Reimplement Vercel's $150/mo Advanced Deployment Protection add-on for free**, on any framework — a self-contained middleware gate (Next.js `proxy.ts`, or Vercel Routing Middleware for SvelteKit/Nuxt/Astro/static). Covers **all three** of the add-on's features — **Password Protection**, **private production deployments** (opt-in; previews by default), and **Deployment Protection Exceptions** (`PREVIEW_GATE_UNPROTECTED_HOSTS` makes specific domains public) — plus a better **Protection Bypass for Automation**: many *named, individually revocable* tokens instead of Vercel's one project-wide secret. Humans unlock once via a signed 1-year cookie (only a **scrypt hash** lives in env). The unlock page is **yours to brand** — Vercel's password screen has no theming hook, so this is something $150/mo can't buy. Keys off `VERCEL_TARGET_ENV` so custom environments (`staging`) are gated too, and the prod build **strips the gate entirely** → zero production cost. Also the only password option on **Hobby**, where Vercel doesn't sell the add-on. Ships both templates + hash/token/strip scripts. |
