...
| [`production-release-gating`](./skills/production-release-gating/SKILL.md) | Stop every merge to `main` from shipping to prod — deploy only on a real release. Two patterns: release-event-driven (deploy on GitHub Release `published`, for GKE/self-hosted) and the Vercel `ignoreCommand` script (exit 1 = build, 0 = skip), with branch-aware previews and monorepo dependency-release handling. Ships both templates. |
