...
| **Codex** | `chatgpt-codex-connector[bot]` | `P1`/`P2` badges | **on-open-only; on-demand via `@codex review`** (observed 2026-07: did **not** auto-re-review pushes; explicit `@codex review` triggered a fresh pass) | re-triggers; learning unverified | `@codex` *(re-trigger observed 2026-07)* | Posts suggestions as a review with P-badged findings; **reacts 👍 when it has nothing** / is satisfied. Responds to `@codex review` / `@codex address`. Re-trigger it after each push you need its sign-off on — it won't come back on its own. Whether a teaching reply changes its future reviews is still unverified. |
...
- **Tag decision — combine the two axes** (re-review cadence × @-tag response; the
  ...
  - **Tag to re-trigger**: **on-demand / on-open-only** cadence rows — post
    `@handle review` after a push when you still need that bot's pass on the new HEAD.
    Never re-trigger-tag **auto-per-push** rows; they re-review themselves and the tag
    only spawns a redundant pass.
