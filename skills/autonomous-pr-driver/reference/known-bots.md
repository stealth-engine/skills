...
| **Cursor Bugbot** | `cursor[bot]` | `BUGBOT_BUG_ID: <uuid>` | **auto-per-push** | **No (observed)** | `@cursor` | Re-posts the *same* `BUGBOT_BUG_ID` against new line numbers every push, including long-fixed ones — dedup by the id. Auto-reviews every push, so you rarely *need* to re-trigger, but **`@cursor review` does force a manual run** and **`@cursor remember [fact]`** teaches a persistent learned rule. Ships "Fix in Cursor/Web" deep-links. Severity: Low/Medium/High. |
...
| **Cursor Bugbot** (`cursor[bot]`, `@cursor`) | `@cursor review` · `cursor review` · `bugbot run` (`cursor review verbose=true` for detailed logs) | **No comment command.** Dashboard only: **"Run only when mentioned"** (silences auto-review until you comment a trigger) or **"Run only once per PR"**. | `@cursor remember [fact]` teaches a persistent learned rule. No comment resolve/dismiss. |
| **Codex** (`@codex`) | **`@codex review`** (flags P0/P1 only) · `@codex review for <focus>` | **No comment command.** Settings only: turn off **Code review** / **Automatic reviews** for the repo. | `@codex <anything-but-`review`>` = a cloud task that *makes changes* (e.g. `@codex fix the P1 issue`), **not** a review. |
...
| **Greptile** (`@greptileai`) | `@greptileai` (mention alone; also `@greptileai <question/focus>`) | **No comment command.** Config only: `triggerOnUpdates: false` (stop per-push re-review) · `skipReview: "AUTOMATIC"` (manual-only) · `disabledLabels` / `excludeBranches` / `ignoreKeywords`. | Too chatty → config `strictness` / `commentTypes` / `updateSummaryOnly`, or 👎-react to train it down (`.greptile/config.json` or dashboard). |
