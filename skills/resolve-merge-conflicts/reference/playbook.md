...
```bash
# Is a PR actually conflicting (vs just behind)? DIRTY = conflicts; BEHIND = just
# needs an update (no conflicts). CONFLICTING/CLEAN is the coarse boolean.
...
# resolved) manifest so it's internally consistent:
...
don't check it out and run its code, and a push to its branch will 403. Confirm and
hand off:
