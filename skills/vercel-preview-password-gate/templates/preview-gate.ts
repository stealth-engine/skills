 ...
 *
 * Human auth: PREVIEW_PASSWORD_HASH holds `s2:<salt>:<scryptHex>`
 ...
 * PREVIEW_GATE_BYPASS_TOKENS holds JSON `{"<label>":"<token>", ...}` — manage
...
function loadGateConfig(): GateConfigState {
  const stored = process.env.PREVIEW_PASSWORD_HASH?.trim();
  if (stored) {
    const match = /^s2:([0-9a-f]+):([0-9a-f]{64})$/i.exec(stored);
    if (match) return { salt: match[1], hash: match[2].toLowerCase() };
    console.warn("[preview-gate] PREVIEW_PASSWORD_HASH is malformed — failing closed");
    return "malformed";
  }
  // Legacy fallback: plaintext env var, normalized to the same scheme.
  const plain = process.env.PREVIEW_PASSWORD;
  if (plain) {
    ...
    if (plain.length > MAX_PASSWORD_LENGTH) {
      console.warn("[preview-gate] PREVIEW_PASSWORD exceeds the max length — failing closed");
      return "malformed";
    }
    return {
...
function bypassTokens(): BypassTokens {
  const raw = process.env.PREVIEW_GATE_BYPASS_TOKENS?.trim();
  if (!raw) return {};
  try {
  ...
  console.warn("[preview-gate] PREVIEW_GATE_BYPASS_TOKENS is malformed JSON — ignoring");
  return {};
}
...
// PREVIEW_GATE_UNPROTECTED_HOSTS="demo.acme.com, staging.acme.com".
  ...
  if (configState === "malformed") {
    return {
      block: new NextResponse(
        "Preview gate misconfigured — PREVIEW_PASSWORD_HASH is not a valid s2 hash, or PREVIEW_PASSWORD exceeds the maximum length.",
